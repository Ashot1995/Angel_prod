            ...
            <div class="card-body">
                ...
                <form class="mb-3" @submit.prevent="validateBeforeSubmit('register')" data-vv-scope="register">
                    <div class="form-group">
                        <label class="text-muted" for="signupInputEmail1">Your name</label>
                        <div class="input-group with-focus">
                            <input :class="{'form-control border-right-0':true, 'is-invalid': errors.has('register.name')}" placeholder="Enter your name" v-model="register.name" v-validate="'required|min:3'" type="text" name="name"/>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...mapActions (['tryRegister']),
            validateBeforeSubmit(scope) {
                this.$validator.validateAll(scope)
                    .then((result) => {
                        if (!result) return Promise.reject('Correct them errors!');

                        const {name, email, password1, password2} = this.register;
